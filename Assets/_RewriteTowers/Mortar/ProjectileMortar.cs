...
public class ProjectileMortar : ProjectileBase
{
    ...
    // note: need to do some better method for getting a proper speed for the projectile if it's out of range
    ...
    public void CalculateArc()
    {
        ...
        if (solve_ballistic_arc(transform.position, speed, target.position, gravity, out impactPos, out fireVel) > 0)
        {
            impulse = fireVel;
            lastPos = transform.position;
            Debug.Log(fireVel);
            //transform.GetComponent<Rigidbody>().velocity = fireVel;
        }
        else
        {
    ...
    // beyond this point is ballistics code
    ...
    public static float ballistic_range(float speed, float gravity, float initial_height)
    {
        ...
        Debug.Assert(speed > 0 && gravity > 0 && initial_height >= 0, "ballistic_range called with invalid data");
    ...
    public static int solve_ballistic_arc(Vector3 proj_pos, float proj_speed, Vector3 target, float gravity, out Vector3 s0, out Vector3 s1)
    {
        ...
        // Handling these cases is up to your project's coding standards
        Debug.Assert(proj_pos != target && proj_speed > 0 && gravity > 0, "solve_ballistic_arc called with invalid data");
